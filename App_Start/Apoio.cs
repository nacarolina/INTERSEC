...
namespace chart
{
    public class Apoio
    {
        ...
        public void CarregaConfigGrupos(string idProgramacao, string anel, string IdPlano)
        {
            ...
            lstEstados.Clear();
            dt = db.ExecuteReaderQuery("SELECT id, Estado, GrupoLogico, Cor, COALESCE(TempoVerde,0) TempoVerde, COALESCE(Amarelo,0) Amarelo, COALESCE(VermelhoIntermitente,0) VermelhoIntermitente, COALESCE(VermelhoLimpeza,0) VermelhoLimpeza, COALESCE(TempoVerdeDemanda,0) TempoVerdeDemanda FROM  Estado" +
