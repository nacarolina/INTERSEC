...
namespace GwCentral.Admin
{
    public partial class MapConfig : System.Web.UI.Page
    {
        Banco db = new Banco("");
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                long idPrefeitura = GetCityHallId();
                DataTable dt = db.ExecuteReaderQuery(string.Format("select latitude,longitude,zoom from ConfigMap where idPrefeitura={0}", idPrefeitura));
                if (dt.Rows.Count > 0)
                {
                    DataRow dr = dt.Rows[0];
                    Page.ClientScript.RegisterStartupScript(this.GetType(),
"LoadMap", "LoadMap('" + dr["latitude"].ToString() + "','" + dr["longitude"].ToString() + "','" + dr["zoom"].ToString() + "');", true);
